...
window.BlazingCalendar = {
    ...
    buildCalendarEventChangeResponse: function (info) {
        ...
        return JSON.stringify(response, BlazingCalendar.replaceNullWithUndefined);
    },
    ...
    interop: {
        ...
        calendarInit: function (elementName, settings) {
            ...
            if (calendarSettings.droppable === true) {
                BlazingCalendar.draggables = new FullCalendarInteraction.Draggable(document.getElementById('external-events'), {
            ...
            // console.log(calendarSettings);
            BlazingCalendar.calendarRef = new FullCalendar.Calendar(calendarEl, calendarSettings);
            // console.log(BlazingCalendar.calendarRef);
            BlazingCalendar.calendarRef.render();
        },
        calendarChangeDuration: function (units, amount) {
            if (BlazingCalendar.calendarRef !== null) {
                BlazingCalendar.calendarRef.setOption('duration', { units: amount });
            }
        },
        calendarSetOption: function (option, value) {
            if (BlazingCalendar.calendarRef !== null) {
                BlazingCalendar.calendarRef.setOption(option, value);
            }
        },
        calendarRefetchResources: function (newCalendarResourceFeed) {
            if (BlazingCalendar.calendarRef !== null) {
                BlazingCalendar.calendarRef.setOption('resources', newCalendarResourceFeed);
                BlazingCalendar.calendarRef.refetchResources();
            }
        },
        calendarRefetchEvents: function () {
            if (BlazingCalendar.calendarRef !== null) {
                BlazingCalendar.calendarRef.refetchEvents();
            }
        },
        calendarOnDrop: function (info) {
            if (BlazingCalendar.calendarRef !== null) {
                BlazingCalendar.interop.callBackDotNetWithInfo('AddEventCallback', info);
                info.remove();
            }
        },
        calendarOnEventResize: function (info) {
            if (BlazingCalendar.calendarRef !== null) {
                BlazingCalendar.interop.callBackDotNetWithInfo('UpdateEventCallback', info);
            }
        },
        calendarOnEventClick: function (info) {
            if (BlazingCalendar.calendarRef !== null) {
                BlazingCalendar.interop.callBackDotNetWithInfo('ClickEventCallback', info);
            }
        },
        callBackDotNetWithInfo: function (functionName, info) {
            if (BlazingCalendar.dotNetReference !== null) {
                BlazingCalendar.dotNetReference.invokeMethodAsync(
                    functionName,
                    BlazingCalendar.buildCalendarEventChangeResponse(info)
                );
            }
        },
        SetDotNetReference: function (ref) {
            BlazingCalendar.dotNetReference = ref;
        },
